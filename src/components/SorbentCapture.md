...
10. The sorbent's Capture Efficiency ($\eta$), defined as the fraction of CO₂ removed from the air, is 7.1%.
    $$\eta = 1 - e^{-k_m \cdot \tau} = 1 - e^{-1.3 \cdot 0.056} \approx 0.071 \text{ or } 7.1\%$$
11. The system's average Capture Rate ($Y$) is 0.12 kg of CO₂ per hour of operation.
    $$Y = (Q \cdot C_{in} \cdot \eta) \cdot 3600 \frac{\text{s}}{\text{hr}} = (0.57 \cdot 0.0008 \cdot 0.071) \cdot 3600 \approx 0.12 \frac{\text{kg}}{\text{hr}}$$
12. The amine sorbent's SotA Working Capacity ($C_w$) is defined as 10% of its mass (0.1 kg of CO₂ per 1 kg of sorbent).
13. The total Weekly CO₂ Capture under this model shall be 6.72 kg.
    $$\text{Weekly Capture} = Y \cdot \text{Daily Runtime} \cdot 7 = 0.12 \frac{\text{kg}}{\text{hr}} \cdot 8 \frac{\text{hr}}{\text{day}} \cdot 7 \text{ days} = 6.72 \text{ kg}$$
14. We assume that filters will be picked up once a week.
15. A single filter cartridge designed to operate for one full week (56 hours) would require 67.2 kg of sorbent and have a total weight of approximately 90 kg.
    $$\text{Sorbent Mass} = \frac{\text{Weekly Capture}}{C_w} = \frac{6.72 \text{ kg}}{0.1} = 67.2 \text{ kg}$$   $$\text{Total Weight} = \frac{\text{Sorbent Mass}}{0.75} \approx 90 \text{ kg}$$
16. For practical user handling, the system shall use modular cartridges with a maximum weight of 10-15 kg.
17. The user will be required to swap between 6 and 9 cartridges weekly to manage the captured CO₂.
    $$\text{Swaps} = \frac{\text{Total Weight}}{\text{Cartridge Weight}} = \frac{90 \text{ kg}}{15 \text{ kg}} \text{ to } \frac{90 \text{ kg}}{10 \text{ kg}} = 6 \text{ to } 9 \text{ cartridges}$$
